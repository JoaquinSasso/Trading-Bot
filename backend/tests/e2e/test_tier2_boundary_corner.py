    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                ...
                -5,
                False,
            ),  # End inside 16m, start outside -> clamped to now-16m -> returns valid slice
        ],
    )
    @pytest.mark.asyncio
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                ...
                0,
                False,
            ),  # 13:00 ET (18:00 UTC) early close on Black Friday -> closed
        ],
    )
...
class TestTier2Feature05AlpacaCalendarBoundary:
    ...
    def test_calendar_special_days(
        self, year: int, month: int, day: int, is_trading_day: bool
    ) -> None:
...
class TestTier2Feature08SyntheticBarGeneratorBoundary:
    ...
    def test_synthetic_bar_zero_volume_integrity(
        self, consecutive_gaps: int, expected_volume: int
    ) -> None:
...
class TestTier2Feature09PriceAdjustmentManagerBoundary:
    ...
    def test_split_adjustment_boundaries(
        self, ratio: float, raw_price: float, expected_adjusted: float
    ) -> None:
...
class TestTier2Feature10PostgresDailyBarsCacheBoundary:
    ...
    @pytest.mark.asyncio
    async def test_cache_boundary_scenarios(
        self, sim_clock: SimulatedClock, test_case: str
    ) -> None:
        provider = MockAlpacaProvider(clock=sim_clock)
...
class TestTier2Feature15RESTFallbackBoundary:
    ...
    def test_rest_polling_interval_adherence(
        self, poll_interval_seconds: float, is_acceptable: bool
    ) -> None:
...
class TestTier2Feature22DiscardMetricsTrackerBoundary:
    ...
    def test_metric_counter_extremes(
        self, initial: int, increment: int, expected_final: int
    ) -> None:
...
class TestTier2Feature34RegimeSnapshotPersistenceBoundary:
    ...
    def test_snapshot_blocking_flag(
        self, sim_clock: SimulatedClock, regime: str, expected_blocked: bool
    ) -> None:
...
class TestTier2Feature35FinnhubEarningsIngestorBoundary:
    ...
    def test_earnings_hour_code_robustness(
        self, raw_hour_code: str, expected_category: str
    ) -> None:
        valid_codes = {
            "bmo": "before_market_open",
            "amc": "after_market_close",
...
class TestTier2Feature39MacroWindowFilterBoundary:
    ...
    def test_macro_blackout_exact_minutes(
        ...
        filter_eng.load_events(
            [{"name": "FOMC", "type": "FOMC", "timestamp": ev_time, "impact": "critical"}]
        )
...
class TestTier2Feature40MacroCalendarExpiryMonitorBoundary:
    ...
    def test_calendar_expiry_boundary_days(
        ...
        if horizon_days > 0:
            filter_eng.load_events(
                [{"name": "FOMC", "timestamp": now + timedelta(days=horizon_days)}]
            )
        else:
            filter_eng.load_events([])
...
class TestTier2Feature41OpenWindowFilterBoundary:
    ...
    def test_open_window_minute_boundaries(
        self, minute: int, second: int, expected_in_window: bool
    ) -> None:
...
class TestTier2Feature42CloseWindowFilterBoundary:
    ...
    def test_close_window_minute_boundaries(
        self, minute: int, is_moc_exit: bool, expected_blocked: bool
    ) -> None:

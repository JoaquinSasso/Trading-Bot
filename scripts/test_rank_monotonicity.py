...
def run_s5_top_n_simulation(
    ...
    """Ejecuta la simulación de Top-N de S5 usando el motor unificado BacktestEngine."""
    ...
    effective_top_n = top_n if top_n is not None else len(all_symbols)
    strat = DualMomentumLeaderStrategy(
        momentum_lookback_days=momentum_lookback_days,
        top_n_leaders=effective_top_n,
    ...
    config = BacktestConfig(
        ...
        max_open_positions=effective_top_n,
        single_position_cap=1.0 / effective_top_n,
        trailing_ema_period=trailing_ema_period,
...
def main() -> int:
    ...
    bucket_df_14, univ_mean_14, _obs_df_14 = analyze_rank_monotonicity_buckets(
        daily_data=daily_14,
        symbols=[s for s in UNIVERSE_14 if s != "SPY"],
    ...
    # -------------------------------------------------------------------------
    bucket_df_gics, univ_mean_gics, _obs_df_gics = analyze_rank_monotonicity_buckets(
    ...
    print("=" * 105)
    print(f"{'Configuración':<26} | {'2020':<8} | {'2021':<8} | {'2022':<8} | {'Trienio':<9} | {'Sharpe':<6} | {'MaxDD':<7}")
    print("-" * 105)
    ...
    for c_name, _ in configs:
        ...
        print(f"{c_name:<26} | {r20:+7.2f}% | {r21:+7.2f}% | {r22:+7.2f}% | {r3y.total_return_pct:+8.2f}% | {r3y.sharpe_ratio:5.2f} | {r3y.max_drawdown_pct:6.2f}%")
    print("=" * 105)
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        ...
        f.write("| Configuración | 2020 | 2021 | 2022 | Trienio 2020-22 | Sharpe | MaxDD |\n")
        f.write("| :--- | :---: | :---: | :---: | :---: | :---: | :---: |\n")
        for c_name, _ in configs:
            r20 = results_matrix[c_name]["2020"].total_return_pct
            r21 = results_matrix[c_name]["2021"].total_return_pct
        ...
        f.write("Muestreo semanal de cada rebalanceo de 2020 a 2022. Cada observación mide el retorno real obtenido a 45 sesiones futuras por el activo que ocupaba ese puesto de ranking.\n\n")
